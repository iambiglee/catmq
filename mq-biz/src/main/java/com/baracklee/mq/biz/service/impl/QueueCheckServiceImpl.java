...
public class QueueCheckServiceImpl implements RedundanceCheckService {
    ...
    public String checkItem() {
        return "Queue下校验:" + QueueEntity.FdTopicId + "_" + QueueEntity.FdTopicName + ", "
    ...
    public String checkQueue(Map<String, TopicEntity> topicMap, Map<Long, QueueEntity> queueMap, Map<Long, DbNodeEntity> dbMap, Map<Long, Long> queueMaxIdMap) {
        ...
        for (long qId : queueMap.keySet()) {
            ...
            if (!dbMap.containsKey(dbNodeEntity.getId())) {
                resultBuilder.append("queue表中的queueId为：").append(qId).append("的queue，对应的db_node_id在dbnode表中不存在！").append("<br/>");
            }
            if (!StringUtils.isEmpty(queueEntity.getTopicName())) {
                if (topicMap.containsKey(queueEntity.getTopicName())) {
                    if (topicMap.get(queueEntity.getTopicName()).getId() != queueEntity.getTopicId()) {
                        resultBuilder.append("queue表中的queueId为：").append(qId).append("的queue，对应的topic：").append(queueEntity.getTopicName()).append("名字和id不一致").append("<br/>");
                    }
                } else {
                    if(!topicService.NEED_DELETED_TOPIC_NANE.equals(queueEntity.getTopicName())){
                        resultBuilder.append("queue表中的queueId为：").append(qId).append("的queue，对应的topic").append(queueEntity.getTopicName()).append("不存在,建议清除该记录中topic的相关信息").append("<br/>");
                    }
