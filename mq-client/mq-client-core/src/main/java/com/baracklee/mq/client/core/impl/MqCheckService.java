...
import com.baracklee.mq.client.core.IMqCheckService;
...
public class MqCheckService implements IMqCheckService {
    ...
    private void checkDataLocalRemote() {
    ...
    private String doCheck(Map<String, Long> map) {
    ...
    private String checkQueues(String groupName,
                               Map<Long, ConsumerQueueDto> queues,
        ...
        for (Map.Entry<Long, ConsumerQueueDto> t1 : queues.entrySet()) {
            ...
            if (!checkDataLocalRemote(t1.getValue().getOriginTopicName(), local.getOriginTopicName())) {
                rs.append("在 consumerGroupName:" + groupName + ",topic:" + t1.getValue().getTopicName()+"下,"
            ...
            if (!checkDataLocalRemote(t1.getValue().getTag(), local.getTag())) {
                rs.append("在 consumerGroupName:" + groupName + ",topic:" + t1.getValue().getTopicName()+"下,"
            ...
            if (!checkDataLocalRemote(t1.getValue().getTopicName(), local.getTopicName())) {
                rs.append("在 consumerGroupName:" + groupName + ",topic:" + t1.getValue().getTopicName()+"下,"
    ...
    private String checkConsumerGroup(ConsumerGroupOneDto t1) {
        if(t1.getMeta()==null) return "meta 为空";
        String rs=checkGroupMeta(t1.getMeta());
        return rs;

    }

    protected boolean checkDataLocalRemote(String remote, String local) {
        return (remote + "").equals(local + "");
    }

    private String checkGroupMeta(ConsumerGroupMetaDto meta) {
