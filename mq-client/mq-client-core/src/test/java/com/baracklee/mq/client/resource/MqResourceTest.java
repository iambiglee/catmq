...
import com.baracklee.mq.biz.common.util.BrokerException;
...
public class MqResourceTest extends AbstractTest {
    ...
    public void constructorTest() {
        ...
        assertEquals("constructorTest error", false, rs);
...
//        mqResource.setUrls(Arrays.asList("1", "2"), null);
        ...
        failUrlG1.put("2", System.currentTimeMillis() - 9 * 1000);
        mqResource.setUrls(Arrays.asList("1"), null);
        rs = mqResource.getHost(true);
...
//		assertEquals("publishNullTest error", true, mqResource.publish(new PublishMessageRequest()));
    ...
    public void postTest() throws IOException, BrokerException {
        ...
        when(httpClient.post(anyString(), anyObject(), eq(BaseResponse.class))).thenReturn(response2);
        assertEquals("sendMail error", null, mqResource.post(new Object(), MqConstanst.CONSUMERPRE + "/sendMail", 2, BaseResponse.class, true));
